...
const layout = (list: ItemObj[], spread: number, lift: number, tilt: number, sizes: { w: number; h: number }[]) => {
...
export default function FolderFloat({
  ...
  const latest = useRef<{
    onSelect?: (val: string, i: number) => void;
    onOpenChange?: (open: boolean) => void;
    drift: number;
    reduce: boolean;
  }>({ drift, reduce: false });
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      setSizes(prev =>
        prev.length === next.length && prev.every((s, i) => s && next[i] && s.w === next[i]!.w && s.h === next[i]!.h) ? prev : next as { w: number; h: number }[]
      );
    };
    measure();
  ...
  const pick = (item: ItemObj, i: number) => {
  ...
  const up = (e: PointerEvent, i: number, item: ItemObj) => {
    const w = world.current;
    const d = w.drag;
    if (!d || d.i !== i || d.id !== e.pointerId) return;
    w.drag = null;
